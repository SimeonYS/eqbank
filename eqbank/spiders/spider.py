...
from ..items import EeqbankItem
from itemloaders.processors import TakeFirst

pattern = r'(\xa0)?'

class EeqbankSpider(scrapy.Spider):
	name = 'eqbank'
	start_urls = ['http://eqbank.investorroom.com/news-releases?l=50']
	...
	def parse_post(self, response, date):
		title = ''.join(response.xpath('//div[@class="wd_title wd_language_left"]//text()').getall())
		content = response.xpath('//div[@class="wd_body wd_news_body"]//text()').getall()
		content = [p.strip() for p in content if p.strip()]
		content = re.sub(pattern, "",' '.join(content))

		item = ItemLoader(item=EeqbankItem(), response=response)
