...
class EeqbankPipeline:

    # Database setup
    conn = sqlite3.connect('eqbank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `eqbank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM eqbank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO eqbank (date, title, link, content)"
